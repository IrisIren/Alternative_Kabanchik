...
public class EcgHandler extends Thread {
    ...
    EcgHandler(BlockingQueue<Double> queue, ConcurrentLinkedQueue<Double> GUIQueue, ConcurrentLinkedQueue<Double> GUIQueue1, AtomicBoolean running) {
...
//        while ((movingWindowIntegration.size() < sample) && (!fileIsFinished.get() || !queue.isEmpty())) {
        while ((movingWindowIntegration.size() < sample) && running.get()) {
            stepping();
            //GUIQueue1.add(0.0);
        }
